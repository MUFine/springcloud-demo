...
import com.m.demo.jwt.UserUtil;
...
/*        if(StringUtils.isNotBlank(token)){
            if(JWTUtil.verifyToken(token,"secret")){
                String loginName = UserUtil.getLoginName(httpServletRequest);
                String userId = UserUtil.getUserId(httpServletRequest);
                if("1".equals(redisTemplate.opsForValue().get(loginName)) && token.equals(redisTemplate.opsForValue().get(userId))){
                    getResult(httpServletResponse,"{\"code\":99,\"msg\":\"用户被删了!\"}");
