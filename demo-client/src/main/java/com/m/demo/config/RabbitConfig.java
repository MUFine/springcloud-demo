...
public class RabbitConfig {
    ...
    public Queue myQueue() {
        return new Queue(CommonQueue.BUY_QUEUE_NAME,true);
    }

    //给Fanout交换机发送消息，绑定了这个交换机的所有队列都收到这个消息,延时交换机
    @Bean
    public CustomExchange customExchange() {
        CustomExchange customExchange = new CustomExchange(CommonQueue.CUSTOM_EXCHANGE, "direct",true,false);
    ...
    public Binding bindingExchange() {
        return BindingBuilder.bind(myQueue()).to(customExchange()).with(CommonQueue.ROUTE_KEY).noargs();
    }
    ...
    public RabbitTemplate rabbitTemplate(ConnectionFactory connectionFactory){
        ...
        rabbitTemplate.setReturnCallback((message, replyCode, replyText, exchange, routingKey) -> {
            System.out.println("消息:" + message + ";应答码:" + replyCode + ";原因:" + replyText + ";交换机:" + exchange + ";路由键:" + routingKey);
        });
